...
int main(int argc, char *argv[]) {
    ...
    printf("allocating %lld bytes (%.2f MB)\n", 
	   size_in_bytes, (double) size_in_bytes / (1024 * 1024.0));
    ...
    unsigned long long int num_ints = size_in_bytes / sizeof(int);
    ...
    while (1) {
	    ...
	    if (i == num_ints) {
	        ...
	        if (time_since_last_print >= 0.2) { // only print every .2 seconds
	    	    printf(
	    	        "loop %d in %.2f ms (bandwidth: %.2f MB/s)\n",
	    	        loop_count,
	    	        1000 * delta_time,
                    (double) size_in_bytes / (1024.0*1024.0*delta_time)
                );
